...
def toy4():
	...
	def f(x):
		# return (-1 * (x ** 4)) + (3 * (x ** 2)) + 1
		return np.cos(x/6)*np.sin(x/9)

	# X_train = torch.tensor([-2.5, -2.3, -2, -1.8, -1.5, 1.3, 1.6, 2, 2.1, 2.2, 2.3, 2.31]).unsqueeze(dim=1)
	n_data = 15
	X_train = torch.from_numpy(100*np.random.random(n_data).reshape(-1, 1)).float()
	Y_train = f(X_train)
	X_test = torch.tensor([-2.4, -1.6, -1.0, -0.3, 0.5, 1.0, 1.4, 1.9]).unsqueeze(dim=1)
	Y_test = f(X_test)
	
	return {"dataset_name": "toy-box", "X_train": X_train/torch.max(X_train), "Y_train": Y_train, "X_test": X_test, "Y_test": Y_test}
