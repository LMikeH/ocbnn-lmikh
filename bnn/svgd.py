...
class BNNSVGD(BayesianNeuralNetwork):
	...
	def infer(self, verbose=True):
		...
		optimizer = torch.optim.Adam([self.particles], lr=self.config["svgd_init_lr"])
...
class BNNSVGDRegressor(BNNSVGD, BNNRegressor):
	...
	def predict_all(self, domain):
		results = []
		for particles, pt in self.all_particles:
			results.append(self.predict(particles, domain).squeeze())

		results = np.array(results).reshape(len(domain), -1)

		return results.mean(axis=1), 2*results.std(axis=1)
